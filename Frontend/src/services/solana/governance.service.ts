...
export class GovernanceService {
  ...
  constructor(provider: AnchorProvider) {
    this.programId = new PublicKey(process.env.NEXT_PUBLIC_GOVERNANCE_PROGRAM_ID || '11111111111111111111111111111111');
  ...
  async getUserVotingPower(user: PublicKey): Promise<BN> {
    try {
      ...
      return new BN(1000000); // 1M tokens
    } catch (error) {
      console.error('Failed to get user voting power:', error);
      return new BN(0);
    }
  }
  ...
  async canUserVote(user: PublicKey, proposal: PublicKey): Promise<boolean> {
    try {
      ...
      return existingVote === null; // Can vote if no existing vote
    } catch (error) {
      console.error('Failed to check if user can vote:', error);
      return false;
    }
  }
  ...
  async canExecuteProposal(proposal: Proposal): Promise<boolean> {
    try {
      ...
      console.error('Failed to check if proposal can be executed:', error);
      return false;
    }
  }
  ...
  async removeEventListeners(): Promise<void> {
    try {
      ...
      console.log('Removing governance event listeners');
    } catch (error) {
