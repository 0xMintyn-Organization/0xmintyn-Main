...
import UbiProgramIdl from '../../idl/ubi_program.json';
...
export class UbiService {
  ...
  constructor(provider: AnchorProvider) {
    this.programId = new PublicKey(process.env.UBI_PROGRAM_ID || UbiProgramIdl.address);
    this.program = new Program(UbiProgramIdl as Idl, this.programId, provider);
    logger.info(`UBI Service initialized with program ID: ${this.programId.toString()}`);
  }
  ...
  async getUbiConfig(): Promise<UbiConfig | null> {
    try {
      ...
      logger.debug('Fetched UBI config from blockchain');
      return config as UbiConfig;
    } catch (error) {
      logger.error('Failed to fetch UBI config:', error);
      return null;
    }
  }
  ...
  async getUserProfile(user: PublicKey): Promise<UserProfile | null> {
    try {
      ...
      logger.debug(`Fetched user profile for ${user.toString()}`);
      return profile as UserProfile;
    } catch (error) {
      logger.error(`Failed to fetch user profile for ${user.toString()}:`, error);
      return null;
    }
  }
  ...
  async getFraudDetection(user: PublicKey): Promise<FraudDetection | null> {
    try {
      ...
      logger.debug(`Fetched fraud detection for ${user.toString()}`);
      return fraudDetection as FraudDetection;
    } catch (error) {
      logger.error(`Failed to fetch fraud detection for ${user.toString()}:`, error);
      return null;
    }
  }
  ...
  async getTreasury(): Promise<Treasury | null> {
    try {
      ...
      logger.debug('Fetched treasury from blockchain');
      return treasury as Treasury;
    } catch (error) {
      logger.error('Failed to fetch treasury:', error);
      return null;
    }
  }
  ...
  async getUserTokenBalance(user: PublicKey): Promise<number> {
    try {
      ...
      logger.error(`Failed to get user token balance for ${user.toString()}:`, error);
      return 0;
    }
  }
  ...
  async canClaimInitialUbi(user: PublicKey): Promise<boolean> {
    try {
      ...
      const canClaim = !profile.initialUbiClaimed && profile.isVerified && !profile.isSuspended;
      logger.debug(`User ${user.toString()} can claim initial UBI: ${canClaim}`);
      return canClaim;
    } catch (error) {
      logger.error(`Failed to check initial UBI eligibility for ${user.toString()}:`, error);
      return false;
    }
  }
  ...
  async canClaimMonthlyUbi(user: PublicKey): Promise<boolean> {
    try {
      ...
      const canClaim = now.sub(profile.lastMonthlyClaim).gte(monthInSeconds);
      logger.debug(`User ${user.toString()} can claim monthly UBI: ${canClaim}`);
      return canClaim;
    } catch (error) {
      logger.error(`Failed to check monthly UBI eligibility for ${user.toString()}:`, error);
      return false;
    }
  }
  ...
  async initializeUser(params: InitializeUserParams): Promise<string> {
    try {
      ...
      logger.info(`Initializing user ${userKeypair.publicKey.toString()}`);
      ...
      logger.info(`User ${userKeypair.publicKey.toString()} initialized successfully. TX: ${tx}`);
      return tx;
    } catch (error) {
  ...
  async claimInitialUbi(params: ClaimUbiParams): Promise<string> {
    try {
      ...
      logger.info(`User ${userKeypair.publicKey.toString()} claiming initial UBI`);
      ...
      logger.info(`Initial UBI claimed successfully for ${userKeypair.publicKey.toString()}. TX: ${tx}`);
      return tx;
    } catch (error) {
  ...
  async claimMonthlyUbi(params: ClaimUbiParams): Promise<string> {
    try {
      ...
      logger.info(`User ${userKeypair.publicKey.toString()} claiming monthly UBI`);
      ...
      logger.info(`Monthly UBI claimed successfully for ${userKeypair.publicKey.toString()}. TX: ${tx}`);
      return tx;
    } catch (error) {
  ...
  async reportFraud(params: ReportFraudParams): Promise<string> {
    try {
      ...
      logger.info(`User ${reporterKeypair.publicKey.toString()} reporting fraud for ${reportedUser.toString()}`);
      ...
      logger.info(`Fraud reported successfully. TX: ${tx}`);
      return tx;
    } catch (error) {
  ...
  async fundTreasury(params: FundTreasuryParams): Promise<string> {
    try {
      ...
      logger.info(`Admin ${adminKeypair.publicKey.toString()} funding treasury with ${amount.toString()}`);
      ...
      logger.info(`Treasury funded successfully. TX: ${tx}`);
      return tx;
    } catch (error) {
  ...
  async verifyUser(params: VerifyUserParams): Promise<string> {
    try {
      ...
      logger.info(`Admin ${adminKeypair.publicKey.toString()} verifying user ${userToVerify.toString()}`);
      ...
      logger.info(`User ${userToVerify.toString()} verified successfully. TX: ${tx}`);
      return tx;
    } catch (error) {
  ...
  async suspendUser(params: SuspendUserParams): Promise<string> {
    try {
      ...
      logger.info(`Admin ${adminKeypair.publicKey.toString()} ${suspend ? 'suspending' : 'unsuspending'} user ${userToSuspend.toString()}`);
      ...
      logger.info(`User ${userToSuspend.toString()} ${suspend ? 'suspended' : 'unsuspended'} successfully. TX: ${tx}`);
      return tx;
    } catch (error) {
  ...
  async toggleProgram(adminKeypair: Keypair, active: boolean): Promise<string> {
    try {
      ...
      logger.info(`Admin ${adminKeypair.publicKey.toString()} toggling program to ${active ? 'active' : 'inactive'}`);
      ...
      logger.info(`Program toggled to ${active ? 'active' : 'inactive'} successfully. TX: ${tx}`);
      return tx;
    } catch (error) {
  ...
  async updateUbiAmounts(params: UpdateUbiAmountsParams): Promise<string> {
    try {
      ...
      logger.info(`Admin ${adminKeypair.publicKey.toString()} updating UBI amounts`);
      ...
      logger.info(`UBI amounts updated successfully. TX: ${tx}`);
      return tx;
    } catch (error) {
  ...
  async listenToEvents(callback: (event: any) => void): Promise<void> {
    try {
      ...
      logger.info('UBI event listeners set up successfully');
    } catch (error) {
  ...
  async removeEventListeners(): Promise<void> {
    try {
      ...
      logger.info('UBI event listeners removed');
    } catch (error) {
