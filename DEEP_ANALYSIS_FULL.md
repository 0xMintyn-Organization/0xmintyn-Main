...
3. **Env:** Ensure `NEXT_PUBLIC_SERVER_URI` points to backend (e.g. `http://localhost:8000/api/v1` or your backend base).
...
- **roleService URLs:** All calls now use `${API_BASE_URL}/role/...` (leading slash before "role") so that with baseUrl like `http://localhost:8000/api/v1`, the path is `/role/dashboard` not `role/dashboard`. This prevents 404 when baseUrl has no trailing slash.
...
- **NEXT_PUBLIC_SERVER_URI:** Should be backend base including /api/v1, e.g. `http://localhost:8000/api/v1` (no trailing slash is safer when concatenating paths).
